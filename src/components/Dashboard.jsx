...
import { DollarSign, TrendingUp, TrendingDown, BarChart3, Activity, Target, RefreshCw, WifiOff } from "lucide-react";
import { CurrencyData } from "../API/CurrencyData"
import { NotificationBell } from "./NotificatioBell"

export default function Dashboard({ userId, metricsData, dataError, selectedAccountTypeDetails, recentTrades,unreadCount, onOpenChat }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100">Trading Dashboard</h1>
          <p className="text-gray-600 dark:text-gray-400 mt-1">Monitor your forex positions and market opportunities.</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">${metricsData?.totalBalance?.toLocaleString() || 'N/A'}</div>
            <div className="flex items-center text-sm text-green-600 mt-1">
              <TrendingUp className="w-4 h-4 mr-1" />+{metricsData?.todayPnLPercent || 1.89}% today
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">+${metricsData?.todayPnL?.toLocaleString() || 'N/A'}</div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Unrealized + Realized</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-gray-900 dark:text-gray-100">{metricsData?.openPositions || 0}</div>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">Active trades</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600 dark:text-green-400">{metricsData?.winRate || 0}%</div>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <p className="font-medium text-gray-900 dark:text-gray-100">{pairData.pair}</p>
                          <p className="text-sm text-gray-500 dark:text-gray-400">Vol: {pairData.volume}</p>
                        </div>
                      </div>
                      <div className="text-right">
                        ...
                        {pairData.changePercent !== 'NaN' && (
                          <div
                            className={`flex items-center text-sm ${parseFloat(pairData.change) >= 0 ? "text-green-600" : "text-red-600"}`}
                          >
                            ...
                            {parseFloat(pairData.change) >= 0 ? "+" : ""}
                            {pairData.change} ({parseFloat(pairData.changePercent) >= 0 ? "+" : ""}
          ...
          <CardContent>
            <div className="space-y-4">
              {Array.isArray(recentTrades) && recentTrades.length > 0 ? (
                recentTrades.sort((a, b) => new Date(b.close_time) - new Date(a.close_time)).map((trade) => (
                  <div key={trade.id} className="p-3 border border-gray-200 dark:border-gray-700 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-2">
                        <Badge variant={trade.type.toLowerCase() === "buy" ? "default" : "destructive"} className="text-xs">
                          {trade.type.toUpperCase()}
                        </Badge>
                        <span className="font-medium text-sm text-gray-900 dark:text-gray-100">{trade.pair}</span>
                      </div>
                ...
                <p className="text-center text-gray-500 dark:text-gray-400 py-4">No recent trades available.</p>
              )}
            </div>
          </CardContent>
        </Card>
